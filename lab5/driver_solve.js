...
function signCoin(blindedCoinHash) {
  console.log("Signing coin...");
  
  // طباعة الكائن للتحقق من نوعه وقيمته
  console.log("blindedCoinHash: ", blindedCoinHash);

  // تحقق من أن blindedCoinHash هو BigInteger وله خاصية abs
  if (!blindedCoinHash || !blindedCoinHash.abs) {
    throw new Error("Invalid blinded coin hash: Must be a BigInteger with an abs method");
  }

  console.log("Blinded Coin Hash is valid.");
  ...
  try {
    ...
    console.error("Error while signing coin: ", error);
    throw new Error("Failed to sign the coin.");
  }
}
...
function acceptCoin(coin) {
  ...
  const isValid = blindSignatures.verify({
    ...
    message: coin.toString(),  // استخدم coin.toString() بدلاً من coin.hash
    key: { n: coin.n, e: coin.e },
  });

  console.log("Coin Signature: ", coin.signature);
  console.log("Coin String (Message): ", coin.toString());
  ...
  let selected = choice === 0 ? leftHashes : rightHashes;
  let identityParts = [];

  for (let i = 0; i < selected.length; i++) {
    let realValue = coin.getRis(choice === 0, i);  // Adjusted this part
    let hashToCheck = utils.hash(realValue);

    if (hashToCheck !== selected[i]) {
      throw new Error("Hash mismatch at index " + i);
    }

    identityParts.push(realValue);
  }
...
function determineCheater(guid, ris1, ris2) {
  for (let i = 0; i < ris1.length; i++) {
    if (ris1[i] !== ris2[i]) {
      let xor = utils.xorStrings(ris1[i], ris2[i]);

      if (xor.startsWith(IDENT_STR)) {
        let id = xor.slice(IDENT_STR.length);
        console.log(`Coin ${guid} was double-spent by ${id}`);
        return;
      } else {
        console.log(`Coin ${guid} was used honestly. Merchant is cheating.`);
        return;
      }
    }
  }

  console.log(`Coin ${guid} used only once. No cheating detected.`);
}
